...
<div class="grid grid-cols-2 gap-x-[3.3rem]">
    <div class="grid gap-y-5">
        ...
        <input id="start-project" class="input text-center text-base" value="{{ old('start-project', $data['p_date_start'] ?? '') }}" type="date" name="start-project" />
    </div>
    <div class="grid gap-y-5">
        <label for="end-project">تاريخ نهاية المشروع <span class="text-red-600">*</span></label>
        <input id="end-project" value="{{ old('end-project', $data['p_date_end'] ?? '') }}" class="input text-center text-base" type="date" name="end-project" />
    </div>
</div>
...
<div class="grid grid-cols-2 gap-x-[3.3rem] my-8">
    <div class="grid gap-y-5">
        ...
        <input id="project-duration" name="project-duration" value="{{ old('project-duration', $data['p_duration'] ?? '') }}" class="input text-center text-base input-disabled" type="text" placeholder="" />
    </div>
    <div class="grid gap-y-5">
        <label for="remaining-duration">المدة المتبقية</label>
        <input id="remaining-duration" name="project-remaining" class="input text-center text-base input-disabled" type="text" value="{{ old('project-remaining', $data['p_remaining'] ?? '') }}" />
    </div>
</div>
...
<div class="grid font-normal text-base my-6 gap-y-5">
    ...
    <textarea name="project-description" class="textarea textarea-lg">{{ old('project-description', $data['p_description'] ?? '') }}</textarea>
</div>
...
<div class="grid grid-cols-2 gap-x-[3.3rem] my-8">
    ...
    <div class="grid gap-y-5">
        ...
        <input class="input text-center text-base" type="number" name="benef_number" value="{{ old('benef_number', $data['p_num_beneficiaries'] ?? '') }}" />
    </div>
</div>
